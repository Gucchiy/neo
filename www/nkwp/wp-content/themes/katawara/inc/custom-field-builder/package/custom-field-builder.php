	...
	class VK_Custom_Field_Builder {
		...
		*/
		...
		public static function form_required() {
			$required = '<span class="required">' . __( 'Required', 'katawara' ) . '</span>';
			return $required;
		}
		...
		public static function form_table( $custom_fields_array, $befor_items = '', $echo = true ) {
			...
			foreach ( $custom_fields_array as $key => $value ) {
				...
				if ( $value['type'] == 'text' || $value['type'] == 'url' ) {
					...
					$form_html .= '<button id="media_' . $key . '" class="cfb_media_btn btn btn-default button button-default">' . __( 'Choose Image', 'katawara' ) . '</button> ';

					// 削除ボタン
					// ボタンタグだとその場でページが再読込されてしまうのでaタグに変更
					$form_html .= '<a id="media_reset_' . $key . '" class="media_reset_btn btn btn-default button button-default">' . __( 'Delete Image', 'katawara' ) . '</a>';

				} elseif ( $value['type'] == 'file' ) {
					$form_html .= '<input name="' . $key . '" id="' . $key . '" value="' . self::form_post_value( $key ) . '" style="width:60%;" />
<button id="media_src_' . $key . '" class="cfb_media_btn btn btn-default button button-default">' . __( 'Select file', 'katawara' ) . '</button> ';
					if ( $post->$key ) {
						$form_html .= '<a href="' . esc_url( $post->$key ) . '" target="_blank" class="btn btn-default button button-default">' . __( 'View file', 'katawara' ) . '</a>';
					}
				}
