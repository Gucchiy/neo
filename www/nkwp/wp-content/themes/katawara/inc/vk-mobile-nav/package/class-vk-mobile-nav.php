...
if ( ! class_exists( 'Vk_Mobile_Nav' ) ) {
	class Vk_Mobile_Nav {
		...
		static function setup_widget() {
			...
			register_sidebar(
				array(
					'name'          => __( 'Mobile Nav Upper', 'katawara' ),
			...
			register_sidebar(
				array(
					'name'          => __( 'Mobile Nav Bottom', 'katawara' ),
		...
		public static function menu_set_html() {
			...
			if ( is_active_sidebar( 'vk-mobile-nav-upper' ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) ) {
					...
					echo '<p>' . sprintf( __( 'This is the widget area.<br>You can set widget item from [ <a href="%s">Appearance > Customize</a> ] Page -> "Widgets" panel -> "Mobile Nav Upper" Panel.', 'katawara' ), admin_url( 'customize.php' ) ) . '</p>';
					echo '<p>' . __( '* This message is displayed only to users with editing authority.', 'katawara' ) . '</p>';
					echo '</div>';
				}
			}
			...
			if ( $menu_vk_mobile ) {
				...
				if ( current_user_can( 'edit_theme_options' ) ) {
					...
					echo '<p>' . sprintf( __( 'Menu is not set.<br>Please set menu from [ <a href="%s">Appearance > Customize</a> ] Page -> "Menus" panel -> Menu Locations "Mobile Navigation".', 'katawara' ), admin_url( 'customize.php' ) ) . '</p>';
					echo '<p>' . __( '* This message is displayed only to users with editing authority.', 'katawara' ) . '</p>';
					echo '</div>';
				}
			}
			...
			if ( is_active_sidebar( 'vk-mobile-nav-bottom' ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) ) {
					echo '<div class="veu_adminEdit alert alert-info">';
					echo '<p>' . sprintf( __( 'This is the widget area.<br>You can set widget item from [ <a href="%s">Appearance > Customize</a> ] Page -> "Widgets" panel -> "Mobile Nav Bottom" Panel.', 'katawara' ), admin_url( 'customize.php' ) ) . '</p>';
					echo '<p>' . __( '* This message is displayed only to users with editing authority.', 'katawara' ) . '</p>';
					echo '</div>';
				}
			}
