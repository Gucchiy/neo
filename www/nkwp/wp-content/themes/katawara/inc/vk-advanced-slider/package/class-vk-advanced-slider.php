			...
			if ( $slide_count ) {
				...
				for ( $i = 1; $i <= $slide_count_max; $i++ ) {
					...
					if ( ! empty( $options[ 'top_slide_image_' . $i ] ) ) {
						...
						$slide_html .= '<div class="l-container">';
						...
						if ( $mini_content_args['link_url'] && $mini_content_args['btn_text'] ) {
							...
							$slide_html .= '.' . $style_class . ' .btn-ghost { border-color:' . $mini_content_args['text_color'] . ';color:' . $mini_content_args['text_color'] . ';' . $box_shadow . $text_shadow . ' }';
